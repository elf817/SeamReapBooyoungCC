  ...
  return (
    <div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-[1.2fr_1fr]">
        <Placeholder label="약도 / 지도 이미지" minHeight={320} className="items-center justify-center md:min-h-[560px]" />
        <div className="px-5 sm:px-12 py-12 sm:py-20 flex flex-col gap-8">
          {CONTACT_ROWS.map((row) => (
            <div key={row.label} className="pb-7 border-b border-deep/[0.14]">
              <div className="mb-3 text-[14px] tracking-[0.22em] text-bronze">{row.label}</div>
              <div className="text-[17px] sm:text-[18px] leading-[1.9] text-ink-soft2 font-light">{row.value}</div>
            </div>
          ))}
        </div>
      </section>
